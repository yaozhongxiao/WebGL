...
glProgram.prototype.setColorArray = function(webgl_context, ColorArray, ColorSize){
    var vbo = webgl_context.createBuffer();
    webgl_context.bindBuffer(webgl_context.ARRAY_BUFFER,vbo);
    webgl_context.bufferData(webgl_context.ARRAY_BUFFER, ColorArray, webgl_context.STATIC_DRAW);

    var gl_Color_ID = webgl_context.getAttribLocation(this._program, glShader.VertexParameter.gl_Color);
    webgl_context.enableVertexAttribArray(gl_Color_ID);
    webgl_context.vertexAttribPointer(gl_Color_ID,ColorSize,webgl_context.FLOAT,true,0,0);
}
